    ...
    if msgs > max_msg then
      ...
      if gbanspamonredis then
        if tonumber(gbanspamonredis) ==  4 and not is_owner(msg) then
          ...
          send_large_msg("chat#id"..msg.to.id, "<code>User [ "..name.." ]"..msg.from.id.." Globally banned (spamming)</code>")
          local log_group = 1 --set log group caht id
          --send it to log group
          send_large_msg("chat#id"..log_group, "<code>User [ "..name.." ] ( @"..username.." )"..msg.from.id.." Globally banned from ( "..msg.to.print_name.." ) [ "..msg.to.id.." ] (spamming)</code>")
        end
      end
      kicktable[user] = true
      msg = nil
